...
import { Play, Pause, RotateCcw, Globe, MessageSquare, Globe as GlobeIcon } from "lucide-react";
...
const translations = {
  zh: {
    scenario: "场景7：HTTP/SSE Web 端与 Agent 通信",
    pause: "暂停",
    play: "播放",
    replay: "重放",
    browserSubtitle: "浏览器 / Web 应用",
    agentSubtitle: "服务端 / Agent 处理",
    steps: [
      "Browser 构造 HTTP 请求",
      "通过 HTTP POST 发送给 Agent",
      "Agent 接收请求并开始处理",
      "Agent 通过 SSE 流式推送响应",
      "Browser 实时接收并展示结果",
    ]
  ...
  en: {
    scenario: "Scenario 7: HTTP/SSE Web to Agent Communication",
    pause: "Pause",
    play: "Play",
    replay: "Replay",
    browserSubtitle: "Browser / Web App",
    agentSubtitle: "Server / Agent Handler",
    steps: [
      "Browser constructs HTTP request",
      "Sends HTTP POST to Agent",
      "Agent receives and starts processing",
      "Agent pushes response via SSE stream",
...
export default function HttpSseAnimation() {
  ...
  const httpPath = "M 272 180 C 420 60, 580 60, 728 180";
  // Agent -> Browser (SSE Stream)
  const ssePath = "M 728 240 C 580 380, 420 380, 272 240";
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        >
          ...
          <AgentCard side="left" title="Browser/Web" subtitle={t.browserSubtitle} icon={Globe} />
          <AgentCard side="right" title="Agent" subtitle={t.agentSubtitle} icon={MessageSquare} />

          <svg className="absolute inset-0 h-full w-full z-[15]" viewBox="0 0 1000 520">
            <defs>
              <linearGradient id="httpGradient" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor="#06b6d4" />
                <stop offset="55%" stopColor="#38bdf8" />
                <stop offset="100%" stopColor="#2563eb" />
              </linearGradient>
              <linearGradient id="sseGradient" x1="1" y1="0" x2="0" y2="0">
                <stop offset="0%" stopColor="#f97316" />
                <stop offset="55%" stopColor="#fb923c" />
                <stop offset="100%" stopColor="#ea580c" />
              </linearGradient>
              <filter id="httpSseSoftGlow" x="-50%" y="-50%" width="200%" height="200%">
              ...
              <marker id="httpArrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth">
                <path d="M0,0 L0,6 L9,3 z" fill="#2563eb" />
              </marker>
              <marker id="sseArrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth">
                <path d="M0,0 L0,6 L9,3 z" fill="#ea580c" />
              </marker>
            </defs>

            {/* HTTP Request: Browser -> Agent */}
            <motion.path
              d={httpPath}
              fill="none"
              stroke="url(#httpGradient)"
              strokeWidth="3.5"
              strokeLinecap="round"
              strokeDasharray="9 10"
              markerEnd="url(#httpArrow)"
              filter="url(#httpSseSoftGlow)"
              animate={playing ? { strokeDashoffset: [0, -19] } : { strokeDashoffset: 0 }}
              transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}
            />

            {/* SSE Stream: Agent -> Browser */}
            <motion.path
              d={ssePath}
              fill="none"
              stroke="url(#sseGradient)"
              strokeWidth="3.5"
              strokeLinecap="round"
              strokeDasharray="9 10"
              markerEnd="url(#sseArrow)"
              filter="url(#httpSseSoftGlow)"
              opacity={0.9}
              animate={playing ? { strokeDashoffset: [0, -19] } : { strokeDashoffset: 0 }}
            ...
            {/* 文字标注 */}
            <text x="500" y="72" fontSize="18" fontWeight="700" fill="#0891b2" textAnchor="middle">
              {language === 'zh' ? 'HTTP POST(request)' : 'HTTP POST(request)'}
            </text>
            <text x="500" y="380" fontSize="18" fontWeight="700" fill="#ea580c" textAnchor="middle">
              {language === 'zh' ? 'SSE Stream(response)' : 'SSE Stream(response)'}
            </text>
          </svg>
