"""Local InMemory runner for Phase 3 No-Matrix Core Flow + MCP + SSE reviewer.

This script runs the entire Phase 3 flow in a single process using the
InMemory transport, so no NATS server is required. It wires together:

1. MCP repo tool bridge.
2. ``release-reviewer`` stream handler.
3. ``matrix.message.received`` -> router-agent -> MCP -> stream reviewer -> ``matrix.message.send``.
...
async def send_progress(
    bus: Bus, env: Envelope, room_id: str, progress_text: str
) -> None:
...
async def reviewer(env: Envelope, writer: StreamWriter) -> None:
    """Mock-backed streaming reviewer (copied from sse_agent for local run)."""
    ...
    present_files = [name for name in files if isinstance(evidence.get(name), str)]
    file_list = ", ".join(present_files) if present_files else "selected files"

    grep_result = str(evidence.get("grep_envelope_md", ""))
    grep_count = len([line for line in grep_result.splitlines() if line.strip()])
...
async def on_message(bus: Bus, env: Envelope) -> None:
    ...
    await send_progress(bus, env, room_id, "Received. Inspecting repository...")

    evidence = await collect_repo_evidence(bus)

    await send_progress(
        bus, env, room_id, "Repo evidence collected. Running streaming review..."
    )
...
async def main() -> None:
    bus = Bus.new(
        WithAgentID("phase3-local-runner"),
    ...
    await runner.start()
    print("[phase3] MCP tool bridge started")
    ...
    try:
        ...
        # Give the streaming reviewer time to emit all deltas (stream lasts ~4-6s).
